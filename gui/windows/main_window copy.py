...
class FileItemWidget(QWidget):
    ...
    def actualizar_texto_label(self,disponible = None):
        ...
        width_base = font_metrics.horizontalAdvance(text_base) + 40
        
        # Espacio adicional para los botones
        
        print(f'Espacio disponible: {disponible} \n tamaño path: {width_path} \n tamaño name: {width_name}')

        
        if disponible <  (width_name ):
            nuevo_estado = "icons"    
        elif disponible < (width_path ):
            nuevo_estado = "name"
        else:
...
class MainWindow(QMainWindow):
    ...
    def resizeEvent(self, event):
        ...
        if disponible < 370:
            return  # No permitir cambios por debajo de 370
        ...
        # Solo aplica cambios si el estado ha cambiado
        if nuevo_estado != self._estado_resize:
            self._estado_resize = nuevo_estado  # Actualiza el estado actual

            if nuevo_estado == "icono":
                ...
                if os.path.exists(icon_path):
                    ...
                    self.btn_open_files.setIconSize(QSize(24, 24))
